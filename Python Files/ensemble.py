...
#To load the model one by one
with open('decision', 'rb') as training_model:
    model = pickle.load(training_model)
...
#To load the model one by one
with open('random', 'rb') as training_model:
    model = pickle.load(training_model)
...
#To load the model one by one
with open('svm', 'rb') as training_model:
    model = pickle.load(training_model)
    
spred=model.predict(vector.toarray())
print(spred)
...
bpred=model.predict(vector.toarray())
print(bpred)
...
heva=int(dpred)+int(bpred)+int(spred)+int(rpred)+int(kpred)
...
#Among Hard and Soft Voting Soft voting gives higher accuracy
...
estimator = [] 
estimator.append(('SVM',SVC(kernel='linear'))) 
...
vot_hard.fit(b.count_train, b.y_train) 
y_pred = vot_hard.predict(b.count_test) 
  
# using accuracy_score metric to predict accuracy 
score = accuracy_score(b.y_test,y_pred) 
